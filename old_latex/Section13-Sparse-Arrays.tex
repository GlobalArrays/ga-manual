...
sparse array using the function \textt{NGA\_Sprs\_array\_duplicate(int s\_a)}
...
The data inside the column blocks can be accessed directly using the
\texttt{NGA\_Sprs\_arrayt\_access\_col\_block(int s\_a, int icol, int **idx, int
**jdx, void **val)}. This function returns pointers to the column block data in
